...
// Controles da janela
document.addEventListener('DOMContentLoaded', () => {
    // Inicializar controles da janela personalizados
...
const loadSettings = () => {
    ...
    if (savedSettings) {
        ...
        // Forzar que la homePage sea about:blank siempre
        browserSettings = { ...loaded, homePage: 'about:blank' };
        applySettings();
    }
};
...
class TabSystem {
    constructor() {
        ...
        this.urlBar = document.querySelector('#urlBarWrapper #urlBar') || document.getElementById('urlBar');
    ...
    init() {
        if (this.newTabBtn) {
            this.newTabBtn.addEventListener('click', () => this.createTab('about:blank'));
        }
        // Criar primeira aba com um delay maior
        setTimeout(() => this.createTab('about:blank'), 500);
    }

    setupUrlBar() {
        // Lista de sitios populares para sugerencias inteligentes
        this.popularSites = [
        ...
        this.autocompleteBox.className = 'autocomplete-box';
        this.autocompleteBox.style.position = 'absolute';
        this.autocompleteBox.style.zIndex = '9999';
        this.autocompleteBox.style.display = 'none';
        
    const wrapper = this.urlBar.closest('#urlBarWrapper') || this.urlBar.parentNode;
    wrapper.style.position = 'relative';
    wrapper.appendChild(this.autocompleteBox);
    

        // Cargar historial
        this.searchHistory = JSON.parse(localStorage.getItem('searchHistory') || '[]');

        // Mostrar sugerencias al escribir (mejorado: busca por dominio y nombre)
        this.urlBar.addEventListener('input', (e) => {
            const value = this.urlBar.value.trim().toLowerCase();
            if (!value) {
                this.autocompleteBox.style.display = 'none';
                return;
            }
            // Sugerencias de sitios populares por dominio o nombre
            const smartSuggestions = this.popularSites.filter(site => {
                try {
                    const urlObj = new URL(site);
                    const domain = urlObj.hostname.replace('www.', '');
                    const name = domain.split('.')[0];
                    return domain.includes(value) || name.includes(value) || site.toLowerCase().includes(value);
                } catch {
                    return false;
                }
            });
            // Sugerencias del historial
            const historySuggestions = this.searchHistory.filter(item => item.toLowerCase().includes(value));
            // Unir y quitar duplicados
            const suggestions = [...smartSuggestions, ...historySuggestions].filter((v, i, a) => a.indexOf(v) === i).slice(0, 7);
            ...
            suggestions.forEach(s => {
                ...
                try {
                    ...
                    const domain = urlObj.hostname.replace('www.', '');
                    label = `<b>${domain}</b> <span style='color:#888;font-size:12px;'>${s}</span>`;
                } catch {}
            ...
            this.autocompleteBox.style.top = (this.urlBar.offsetTop + this.urlBar.offsetHeight) + 'px';
            this.autocompleteBox.style.left = this.urlBar.offsetLeft + 'px';
            this.autocompleteBox.style.width = this.urlBar.offsetWidth + 'px';
            this.autocompleteBox.style.display = 'block';
        });

        // Ocultar sugerencias al perder foco
        this.urlBar.addEventListener('blur', () => {
            setTimeout(() => this.autocompleteBox.style.display = 'none', 200);
        });

        // Enter para buscar o navegar
        this.urlBar.addEventListener('keypress', async (e) => {
            if (e.key === 'Enter') {
                ...
                if (webview) {
                    ...
                    if (input && !this.searchHistory.includes(input)) {
                        this.searchHistory.unshift(input);
                        if (this.searchHistory.length > 50) this.searchHistory = this.searchHistory.slice(0, 50);
                        localStorage.setItem('searchHistory', JSON.stringify(this.searchHistory));
                    }
                    // Si es URL válida o dominio, navegar
                    if (this.isValidUrl(input) || this.isDomain(input)) {
                        ...
                        webview.loadURL(url).catch(() => {
                            // Si falla, buscar en Google
                            this.doSearch(webview, input);
                        });
                    } else {
                        // Siempre buscar en Google si no es URL
                        this.doSearch(webview, input);
                    }
                }
                this.autocompleteBox.style.display = 'none';
    ...
    createTab(url) {
        // Si la URL es 'about:blank', cargar la página personalizada
        if (!url || url === browserSettings.homePage || url === 'about:blank') {
        ...
        // Si es start.html, usar ruta absoluta local
        let src = url;
        if (url === 'start.html') {
            src = 'start.html';
        }
        content.innerHTML = `<webview src="${src}" allowpopups></webview>`;
        ...
        webview.addEventListener('dom-ready', () => {
            ...
            if (this.activeTabId === tabId) {
                // Si es la página principal personalizada, ocultar la barra de búsqueda/url
                if (webview.getURL().endsWith('about-blank.html')) {
